 ...
 */
function sendGabShare(body, url, nsfw, callback) {
    if (!body || !url) return callback(false, {});

    console.log("SENDING -SHARE- NOW:", BASE_URI, body, url, nsfw);
 ...
 * @return {Boolean}
 */
function requestGabShareStatus(originalData) {
    //Must have data
    ...
    performRequest({
        ...
        switch (postStatus) {
            ...
            case POST_STATUS_COMPLETE:
                ...
                var url = makeGabSharePostUrlWithShareData(resultShareObject);
 ...
 * @return {String|null}
 */
function makeGabSharePostUrlWithShareData(shareData) {
    //Must be object
